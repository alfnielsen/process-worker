...
export type LogId = { id: string } | string
 ...
 type ILogObject<TData extends any = any> = {
   id: string
   type: string
   level: string
   message: string
...
export type LogQueueEventHandler = (action: ILogObject) => void | boolean | Promise<void | boolean>
...
> implements ILogObject<TArg, TData, TError, TOutput>
{
  // data
  name: string
  ...
  get json(): ILogObject<TArg, TData, TError, TOutput> {
    return {
      id: this.id,
      name: this.name,
  ...
  static create<
    ...
    TOutput extends object | undefined = undefined,
  >(
    opt: { name: string } & Partial<ILogObject<TArg, TData, TError, TOutput>>,
  ...
  private constructor(
    action: ILogObject<TArg, TData, TError, TOutput> | IActionObjectWithEvents<TArg, TData, TError, TOutput>,
    repo: ActionRepo = ActionRepo.createRepo()
  ) {
    this._repo = repo;
